...
public class RedisConfig implements CachingConfigurer {
    ...
    public CacheManager cacheManager(
        ...
        RedisCacheConfiguration defaults = RedisCacheConfiguration.defaultCacheConfig()
                ...
                .prefixCacheNameWith("stockpro:warehouse:")
                .serializeKeysWith(RedisSerializationContext.SerializationPair.fromSerializer(new StringRedisSerializer()))
                .serializeValuesWith(RedisSerializationContext.SerializationPair.fromSerializer(redisValueSerializer));

        Map<String, RedisCacheConfiguration> cacheTtls = Map.of(
                "warehousesAll", defaults.entryTtl(Duration.ofMinutes(5)),
                "stockLow", defaults.entryTtl(Duration.ofMinutes(1)),
                "stockByWarehouse", defaults.entryTtl(Duration.ofMinutes(2)),
                "stockAll", defaults.entryTtl(Duration.ofMinutes(2))
        );
